...
def _defaults(profile: str, base_dir: Path) -> dict[str, Any]:
    ...
    common: dict[str, Any] = {
        ...
        # внешний адрес на странице (CLAUDE.md, «Внешние запросы»). Один запрос
        # на загрузку и только у вошедшего, дальше картинку держит кэш браузера.
        # Выключатель нужен закрытому контуру и тем, кто не хочет отдавать этот
        # запрос наружу; буква в кружке работает в обоих случаях — она видна,
        # пока картинка грузится, и остаётся, если картинка не пришла.
