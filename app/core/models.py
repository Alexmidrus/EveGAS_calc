...
class CalcInput:
    ...
    # «Только buy» — зеркальный фильтр: показать одни заявки. Отдельным флагом,
    # а не множеством сторон: множество сломало бы контракт формы и настройки,
    # сохранённые прежними версиями. Оба флага разом — ошибка, а не пустая
    # выдача: их проверяет и форма, и build_scenarios.
    buy_only: bool = False
