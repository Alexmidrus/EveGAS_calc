...
def test_no_external_font_requests(css: str) -> None:
    """Шрифты раздаются со своего хоста (SPEC §10.1).

    Не потому, что внешние запросы запрещены — они не запрещены, — а потому,
    что `<link>` на CDN означает запрос к чужому серверу на каждую загрузку
    страницы у каждого пользователя, а не загрузившийся шрифт — прыгающую
    вёрстку. Файлом это одна раздача и ноль зависимости от чужой доступности.
    """
